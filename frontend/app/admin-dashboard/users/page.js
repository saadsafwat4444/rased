...
//       setSupervisors(Array.isArray(data) ? data.filter(u => u.role === "Supervisor") : []);
//     } catch (err) {
//       console.error(err);
//       setSupervisors([]);
//     } finally {
...
//     if (!formData.stationScope.length) newErrors.stationScope = "Select at least one station";
...
//     const supervisorData = { ...formData };
//     try {
//       const token = await user.getIdToken();
//       const res = await fetch(`${API_BASE_URL}/users/add-supervisor`, {
//         method: "POST",
...
//         return;
//       }
//       setSupervisors(prev => prev.map(u => u.id === selectedUser.id ? { ...u, role: editRoleValue } : u));
//       setSelectedUser(null);
//       setShowEditModal(false);
//       alert("Role updated successfully!");
//     } catch (err) {
...
//       if (!res.ok) {
//         const errorText = await res.text();
//         alert("Failed to delete supervisor: " + errorText);
//         return;
//       }
...
//   // ================= Pagination =================
//   const indexOfLastUser = currentPage * supervisorsPerPage;
//   const indexOfFirstUser = indexOfLastUser - supervisorsPerPage;
//   const currentUsers = supervisors.slice(indexOfFirstUser, indexOfLastUser);
//   const totalPages = Math.ceil(supervisors.length / supervisorsPerPage);

//   if (loading) return <p className="text-center mt-10 text-gray-300">Loading supervisors...</p>;
//   if (!supervisors.length) return <p className="text-center mt-10 text-gray-300">No supervisors found.</p>;

//   return (
//     <div className="p-4 md:p-10 bg-gray-900 text-gray-100 min-h-screen">
//       <div className="flex justify-between items-center mb-6">
//         <h1 className="text-3xl font-bold text-indigo-400">Supervisor Management</h1>
//         <button onClick={() => setShowAddModal(true)} className="bg-blue-500 px-4 py-2 rounded text-white">
//           Add Supervisor
//         </button>
//       </div>
...
//               <th className="p-3 text-left text-gray-300">Full Name</th>
//               <th className="p-3 text-left text-gray-300">Email</th>
//               <th className="p-3 text-left text-gray-300">Phone</th>
//               <th className="p-3 text-left text-gray-300">Stations</th>
...
//                   <button onClick={() => { setSelectedUser(user); setEditRoleValue(user.role); setShowEditModal(true); }} className="bg-yellow-400 px-3 py-1 rounded text-white">Edit Role</button>
//                   <button onClick={() => handleDelete(user.id)} className="bg-red-500 px-3 py-1 rounded text-white">Delete</button>
//                 </td>
//               </tr>
//             ))}
//           </tbody>
...
//       {/* Pagination */}
//       {supervisors.length > supervisorsPerPage && (
//         <div className="flex justify-between items-center mt-6">
//           <div>Showing {indexOfFirstUser + 1} to {Math.min(indexOfLastUser, supervisors.length)} of {supervisors.length}</div>
//           <div className="flex gap-2">
//             <button disabled={currentPage===1} onClick={()=>setCurrentPage(p=>p-1)} className="px-4 py-2 bg-gray-700 rounded disabled:opacity-50">Previous</button>
//             <span className="px-4 py-2">{currentPage} / {totalPages}</span>
//             <button disabled={currentPage===totalPages} onClick={()=>setCurrentPage(p=>p+1)} className="px-4 py-2 bg-gray-700 rounded disabled:opacity-50">Next</button>
//           </div>
//         </div>
//       )}
...
//             <input placeholder="Full Name" value={formData.fullName} onChange={(e)=>setFormData({...formData, fullName:e.target.value})} className="w-full p-2 border border-gray-600 rounded mb-1 bg-gray-800 text-white placeholder-gray-400"/>
//             {errors.fullName && <p className="text-red-400">{errors.fullName}</p>}

//             <input placeholder="Email" value={formData.email} onChange={(e)=>setFormData({...formData, email:e.target.value})} className="w-full p-2 border border-gray-600 rounded mb-1 bg-gray-800 text-white placeholder-gray-400"/>
//             {errors.email && <p className="text-red-400">{errors.email}</p>}

//             <input type="password" placeholder="Password" value={formData.password} onChange={(e)=>setFormData({...formData, password:e.target.value})} className="w-full p-2 border border-gray-600 rounded mb-1 bg-gray-800 text-white placeholder-gray-400"/>
//             {errors.password && <p className="text-red-400">{errors.password}</p>}

//             <input placeholder="Phone (11 digits)" value={formData.phone} onChange={(e)=>setFormData({...formData, phone:e.target.value})} className="w-full p-2 border border-gray-600 rounded mb-1 bg-gray-800 text-white placeholder-gray-400"/>
//             {errors.phone && <p className="text-red-400">{errors.phone}</p>}
...
//                   <input type="checkbox" checked={formData.stationScope.includes(st.id)} onChange={(e)=>{
//                     if(e.target.checked) setFormData({...formData, stationScope:[...formData.stationScope, st.id]});
//                     else setFormData({...formData, stationScope: formData.stationScope.filter(id=>id!==st.id)});
//                   }}/>
//                   <label>{st.name}</label>
//                 </div>
//               ))}
...
//               <button onClick={handleAddSupervisor} className="bg-blue-500 px-4 py-2 rounded">Add</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//               <button onClick={handleEditRole} className="bg-blue-500 text-white px-4 py-2 rounded">Save</button>
//             </div>
//           </div>
//         </div>
//       )}
