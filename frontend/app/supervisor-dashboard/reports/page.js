...
export default function SupervisorReports() {
  ...
  const fetchReports = async () => {
    try {
      ...
      const data = await apiFetch("/users/reports", {
        headers,
      });
  ...
  const updateReportStatus = async (reportId, newStatus) => {
    try {
      ...
      await apiFetch(`/users/reports/${reportId}/status`, {
        method: "PATCH",
        headers,
        body: JSON.stringify({ status: newStatus }),
      });
  ...
  const assignReport = async (reportId, technicianId) => {
    try {
      ...
      await apiFetch(`/users/reports/${reportId}/assign`, {
        method: "PATCH",
        headers,
  ...
  const addComment = async (reportId) => {
    ...
    try {
      ...
      await apiFetch(`/users/reports/${reportId}/comments`, {
        method: "POST",
        headers,
        body: JSON.stringify({ comment }),
      });
      ...
      {reports.map((r) => (
        <div key={r.id} className="bg-gray-800 p-4 rounded space-y-2">
          <div className="flex justify-between">
            <span>{getStationName(r.stationId)}</span>
